...
const ProductSection = ({ title }) => {
  ...
  return (
    <div className="w-full mb-10">
      ...
      {!isLoading && !error && (
        <div
          className="flex overflow-x-auto pb-6 -mx-6 px-6 md:grid md:grid-cols-3 lg:grid-cols-6 gap-4 xl:mx-0 xl:px-0 [&::-webkit-scrollbar]:hidden"
          style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
        >
          {products.map((product) => (
            <div
              key={product.id}
              className="min-w-[160px] bg-[#212b1b] rounded-2xl p-4 border border-white/5 hover:border-[#dac889]/30 transition-all duration-300 group flex flex-col shadow-lg hover:-translate-y-1.5 focus-within:ring-2 focus-within:ring-[#dac889]/40"
            >
              <div className="bg-white rounded-[14px] p-3 mb-4 flex items-center justify-center aspect-square shadow-inner overflow-hidden border border-gray-100">
                <img
                  src={product.image}
                  alt={product.name}
                  className="object-cover w-full h-full mix-blend-multiply group-hover:scale-110 transition-transform duration-500 ease-out"
                />
              </div>

              <div className="flex flex-col flex-1">
                <span className="text-gray-400 text-xs font-semibold mb-1 tracking-wide">
                  {product.qty}
                </span>
                <h4 className="text-white text-sm font-bold leading-snug mb-3 group-hover:text-gray-200 line-clamp-2 h-10">
                  {product.name}
                </h4>
